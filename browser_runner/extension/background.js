...
async function renderStatusOverlay(text, tone = 'info') {
  return runInTab(
    (value, kind) => {
      ...
      el.textContent = `[Browser Runner] ${String(value || '')}`;
      return { ok: true };
    },
    [text, tone],
  );
...
function connect() {
  ...
  try {
    ...
    console.error('[browser-runner-ext] websocket constructor failed:', String(err?.message || err));
    connecting = false;
    scheduleReconnect();
    return;
  }
  ...
  socket.addEventListener('error', () => {
    console.error('[browser-runner-ext] websocket error');
    connecting = false;
    try {
      socket.close();
    } catch (_) {
