...
export function launchChromium({ browserBinary, showUi, noSandbox = false }) {
  ...
  if (showUi && !display) {
    throw new Error('show_ui=true requires DISPLAY to be set');
  }

  const userDataDir = `/tmp/browser-runner-profile-${Date.now()}-${process.pid}-${Math.floor(Math.random() * 100000)}`;
  const args = [
...
export async function startBrowserEnvironment({ browserBinary, showUi }) {
  ...
  const startWithNoSandbox = runningInDocker;
