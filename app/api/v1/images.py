...
def _build_etag(image_id: int, created_at: datetime | None, source_url: str | None) -> str:
    ...
    if created_at is not None:
        ...
        created_ts = str(int(created_at.timestamp()))
...
def get_image(image_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    repo = ParserImageAssetRepository(db)
    asset = repo.get_by_id(image_id)

    if not asset or asset.deleted_at is not None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Изображение не найдено")

    etag = _build_etag(asset.id, asset.created_at, asset.source_url)
    headers = _cache_headers(etag, asset.created_at)
    ...
    _ensure_allowed_url(asset.source_url)
...
def backfill_image_assets(db: Session = Depends(get_db)):
    ...
    for product in products:
        ...
        if not urls:
            continue
        assets = repo.ensure_assets(urls)
