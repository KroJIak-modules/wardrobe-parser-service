...
class FourteenthaddictionV1Adapter:
    ...
    def _first_image(raw_product: dict) -> str:
        ...
        image_url = FourteenthaddictionV1Adapter._normalize_image_url(str(raw_product.get('image_url') or '').strip(), base_url)
        if image_url:
            return image_url
        ...
        if isinstance(images, list) and images:
            ...
            if isinstance(first, dict):
                return FourteenthaddictionV1Adapter._normalize_image_url(str(first.get('src') or '').strip(), base_url)
            return FourteenthaddictionV1Adapter._normalize_image_url(str(first).strip(), base_url)
        return ''
